// script creates reduced signal ntuple for final fitting of Bmass.
// name of the outpout file to be specified
...
void red_Signal_BDT(){
  ...
  const std::string outfile   = "PhiMuMu_MC_ForFit.root";
  const std::string treename  = "tree";

  TCut b       = "Bmass>4.9 && Bmass<5.8";
  //TCut t       = "(LMNTTriggers==1) && (LMNTTriggersdr0<100 || LMNTTriggersdr1<100)";
  ...
  TCut mycuts = b && t && t1 && t2 && bl && r1 && r2 && phi && bdt && ar && trk;

  TChain *ch_sig = new TChain("tree");
  //ch_sig->Add("./MC/sel_BsToPhiMuMu_OfficialMC_signal_2016Mini_Presel_mc.lite_cut_bdt-s01.root");
  ch_sig->Add("./sel_BsToPhiMuMu_OfficialMC_signal_2016Mini_Presel_mc.lite_cut_bdt-s01.root");
